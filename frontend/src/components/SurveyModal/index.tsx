"use client"

import {
    Box,
    Button,
    Center,
    CloseButton,
    Fieldset,
    Group,
    Select,
    Slider,
    Space,
    Stack,
    Tabs,
    Text,
    Textarea,
    Title,
} from "@mantine/core"
import { useModals } from "@mantine/modals"
import Image from "next/image"
import { useCallback, useRef, useState } from "react"
import type { UseFormReturn } from "react-hook-form"
import { Controller, useForm, useWatch } from "react-hook-form"
import type { SurveyFormData, SurveySchemaItem, TabProps } from "@/types"
import { CheckboxList } from "./CheckboxList"
import { CheckboxWithOther } from "./CheckboxWithOther"

const states = {
    start: "Отправить ответ",
    fetch: "Отправка...",
    ok: "Готово",
    error: "Ошибка, еще раз?",
}

const schema = [
    // tab 1
    {
        id: "e431953d-d694-424c-ac88-f47acad9d629",
        text: "Вы проживаете на территории Соснового Бора?",
        type: "select",
        data: ["Да", "Нет"],
    },
    {
        // IMAGE QUESTION
        id: "a7b52f44-cc92-4df5-bf26-f45a66fe5cb0",
        text: "В каком микрорайоне Соснового Бора вы живете??",
        type: "textarea",
    },
    {
        id: "83e46ab6-5e0a-485a-be88-447ac72c18b0",
        text: "Если вы не проживаете в Сосновом Бору, то с какими целями вы посещаете эту территорию?",
        type: "checkboxOther",
        data: [
            "Работа",
            "Учеба",
            "Отдых на даче",
            "Встречи с друзьями и родственниками",
            "Получение медицинских услуг",
            "Посещение Андерсенграда",
            "Занятие спортом",
            "Прогулки по городу, паркам и берегу Финского залива",
            "Посещение культурно-досуговых и спортивных мероприятий",
        ],
        // maxValues: 3,
        // description: 'Выберите не более трех целей',
    },
    {
        id: "8bfa60be-40c9-460b-ac46-d1371282b40a",
        text: "Какие места и локации Соснового Бора вы посещаете?",
        type: "textarea",
    },
    // tab 2
    {
        id: "ab2c6a06-9e4a-4463-959d-a65e234e895d",
        text: "С чем у вас ассоциируется Сосновый Бор?",
        type: "textarea",
        description: "Напишите несколько слов, которые приходят на ум, когда вы думаете о городе",
    },
    {
        id: "82e32f25-4518-48f1-80b4-0cd3c266d40d",
        text: "Вам комфортно жить и работать в Сосновом Бору?",
        type: "select",
        data: ["Комфортно", "Скорее да", "Скорее нет", "Не комфортно", "Затрудняюсь ответить"],
    },
    {
        id: "27bbb001-8e51-45fd-984c-432b67359f5a",
        text: "Как вы считаете, чем Сосновый Бор лучше других городов?",
        type: "checkboxOther",
        description: "Укажите 5 наиболее значимых моментов",
        data: [
            "Доступность и качество образования",
            "Доступность и качество медицины",
            "Разнообразие культурных и спортивных мероприятий",
            "Близость к Санкт-Петербургу",
            "Транспортная связанность с другими населенными пунктами",
            "Доступность и качество жилья",
            "Качество городской среды и благоустройство",
            "Историческое наследие",
            "Сильное сообщество жителей города и дух соседства",
            "Спокойная и безопасная обстановка",
            "Уникальная природа",
            "Экологическая чистота",
            "Высокий уровень заработных плат",
            "Высокий уровень развития малого и среднего бизнеса",
            "Высокий уровень развития промышленности",
            "Высокий уровень развития туризма",
        ],
        maxValues: 5,
    },
    {
        id: "db1371ea-239d-4d62-8b43-b6e84012fada",
        text: "Какие недостатки выделяют Сосновый Бор среди других городов?",
        type: "checkboxOther",
        description: "Укажите 3 наиболее значимых момента",
        maxValues: 3,
        data: [
            "Плохая транспортная связанность с другими населенными пунктами",
            "Низкая доступность и низкое качество жилья",
            "Низкое качество городской среды и благоустройства",
            "Слабое сообщество жителей города и дух соседства",
            "Недоступность образовательных услуг",
            "Недостаточное качество образования",
            "Недоступность медицинских услуг",
            "Недостаточное качество медицинских услуг",
            "Отсутствие разнообразных культурных мероприятий",
            "Отсутствие разнообразных спортивных мероприятий",
            "Небезопасная обстановка",
            "Загрязнение природы",
            "Низкий уровень заработных плат",
            "Низкий уровень развития малого и среднего бизнеса",
            "Низкий уровень развития промышленности",
            "Низкий/чрезмерный уровень развития туризма",
        ],
    },
    {
        id: "fd8df1fe-e05e-41f7-ad06-7d6fdeb8e498",
        text: "Какие места в Сосновом Бору вам нравятся?",
        type: "textarea",
        description:
            "Вы можете указать любое место или заведение в городе, в котором вам нравится проводить свое время или которое вызывает у вас приятные ощущения",
    },
    {
        id: "086da40d-5732-4ddb-937c-add4652a1beb",
        text: "Какие особенности жизни в Сосновом Бору вы можете выделить?",
        type: "textarea",
        description:
            "Это могут быть любые значимые для вас или просто интересные особенности города (в том числе в людях), которые отличают Сосновый Бор от других городов",
    },

    // tab 3
    {
        id: "eec713f2-d3c2-4091-ade5-d0405292a532",
        text: "Где вы предпочитаете проводить свое свободное время вне дома?",
        type: "textarea",
        description: "Вы можете указать любое место или заведение, в том числе вне Соснового Бора",
    },
    {
        id: "dd7afc2c-48cf-4c18-aba7-91457c884c3c",
        text: "Как часто вы посещали в Сосновом Бору следующие объекты за последний год?",
        type: "selectList",
        list: [
            "Места вблизи водоемов (залив, реки, озера), лесов и других природных объектов",
            "Городские парки, скверы, аллеи, бульвары, площади",
            "Торговые центры и рынок",
            "Медицинские учреждения",
            "Кафе и рестораны",
            "Творческие кружки и секции",
            "Волонтерские центры",
            "Культурные учреждения (культурно-досуговые учреждения, библиотеки, музеи, кинотеатр, театр)",
            "Спорткомплексы, спортзалы и спортивные секции",
            "Детские площадки",
            "Библиотеки",
            "Церковь",
            "Уличные спортивные площадки (кроме школьных)",
        ],
        data: [
            "Ежедневно",
            "Несколько раз в неделю",
            "Несколько раз в месяц",
            "Несколько раз за полгода",
            "Раз в год",
            "Не посещал(а)",
        ],
    },
    {
        id: "dc915fd5-2ead-46f7-9f70-75f6e79cd840",
        text: "Как вы проводили свой отпуск или длинные праздники последние 2 года?",
        type: "checkboxOther",
        description: "Укажите максимум 3 вида отдыха, которые были последними",
        data: [
            "Остался во время отдыха в Сосновом Бору",
            "Выезжал на дачу или в ближайшие базы отдыха, гостевые дома и турбазы",
            "Посещал Санкт-Петербург",
            "Путешествовал по близлежащим городам",
            "Посещал города, где живут друзья или родственники",
            "Отдыхал в санаториях",
            "Выезжал на природу, ходил в походы",
            "Использовал отпуск для дополнительной подработки",
        ],
        maxValues: 3,
    },
    {
        id: "3265ccb0-2174-4097-961b-dbe5d8eaee19",
        text: "Где вы проводили свой отпуск в последние 2 года?",
        description: "Вы можете указать название места, город, страну, которые вы посещали во время отпуска",
        type: "textarea",
    },
    {
        id: "9f2e3d2a-620e-45af-b7af-d30681b2748e",
        text: "Если вы принимаете участие в общественных организациях (профсоюзы, НКО, волонтерские организации, группы по интересам), укажите их название и вашу сферу интересов",
        type: "textarea",
    },
    {
        id: "f05f3f7e-1d4f-4c09-a458-8aa01ea964a6",
        text: "Каких мероприятий в Сосновом Бору вам не хватает?",
        type: "checkboxOther",
        description: "Выберите не более 3-х вариантов.",
        data: [
            "Городские праздники, массовые мероприятия на весь город",
            "Мероприятия для всей семьи, родителей и детей",
            "Развлекательные мероприятия для детей и молодежи",
            "Мероприятия для пожилых людей",
            "Ярмарки и рынки",
            "Гастрономические фестивали",
            "Фестивали ремесел и культуры",
            "Музыкальные мероприятия и концерты",
            "Театральные постановки",
            "Художественные выставки",
            "Спортивные мероприятия и соревнования",
            "Образовательные мероприятия, лекции, мастер-классы",
            "Профессиональные конференции, форумы и лекции",
            "В Сосновом Бору проводят достаточно мероприятий",
        ],
        maxValues: 3,
    },
    // tab 4
    {
        id: "91388a08-4c97-44a7-b8d0-57d908cb3de9",
        text: "Оцените от 0 до 5, насколько вы удовлетворены следующими сферами жизни в Сосновом Бору?",
        type: "sliderList",
        description:
            "Где 1 – абсолютно не удовлетворен(-а), 5 – абсолютно удовлетворен(-а). 0 – не могу оценить (нет таких объектов в городе или не имею представление о развитии данной сферы).",
        list: [
            "Детские сады",
            "Школы",
            "Детский досуг (детские площадки, развлекательные центры, развивающие и спортивные занятия)",
            "Среднее-профессиональное образование",
            "Высшее образование",
            "ФГБУЗ ЦМСЧ №38 ФМБА России",
            "Частные медицинские центры",
            "Культура (культурно-досуговые учреждения, музеи, библиотеки)",
            "Спорт (спортивные комплексы и площадки, залы, секции)",
            "Жилищные условия (состояние дома и благоустройство придомовой территории)",
            "Коммунальные услуги (газ, электричество, отопление, водоснабжение и водоотведение)",
            "Детские площадки, места для прогулок и отдыха с детьми",
            "Рекреационные и общественные пространства (парки, скверы, бульвары и аллеи)",
            "Гаражи и парковочные пространства",
            "Озеленение общественных пространств",
            "Уличное освещение",
            "Безопасность",
            "Велодорожки и велосипедная инфраструктура",
            "Общественное питание (кафе, рестораны, столовые)",
            "Объекты бытового обслуживания (аптеки, парикмахерские, салоны красоты, банки, автосервисы, и т.д.)",
            "Объекты розничной торговли (продуктовые магазины, торговые центры, и т.д.)",
            "Экология (чистота воздуха, воды, почвы, радиационное загрязнение)",
            "Сбор, вывоз и утилизация мусора (в том числе несанкционированные свалки)",
        ],
        marks: [0, 1, 2, 3, 4, 5],
    },
    {
        id: "ea6e3b67-4215-4573-9a0b-ff81a6ea8ddf",
        text: "С какими проблемами вы лично сталкивались в Сосновом Бору за последние 2 года?",
        type: "checkboxList",
        description: "Выберите не более 5 наиболее значимых проблем",
        data: [
            "Сложность получения мест в детских садах",
            "Сложность получения мест в школах",
            "Недостаток возможностей получения высшего образования",
            "Недоступность услуг здравоохранения",
            "Низкое качество услуг здравоохранения",
            "Недостаточность инфраструктуры для занятий спортом и физической культурой",
            "Недостаток услуг в сфере культуры и досуга",
            "Недостаточность или низкое качество работы общественного транспорта",
            "Недоступность качественного жилья",
            "Высокая стоимость покупки и аренды жилья",
            "Низкое качество услуг ЖКХ (вода, газ, отопление, сбор и вывоз мусора)",
            "Низкое качество содержания улиц и придомовых территорий",
            "Недостаточность или низкое качество дорог",
            "Низкое качество подключения к сети Интернет",
            "Недоступность государственных услуг в цифровом (онлайн) формате или их неудовлетворительная работа",
            "Недостаточный ассортимент или низкое качество продуктов питания",
            "Плохая экологическая обстановка",
            "Сложности ведения предпринимательской деятельности",
            "Сложности с поиском работы на местном рынке труда",
            "Не сталкивался с подобными проблемами",
        ],
        maxValues: 5,
    },
    {
        id: "65daf358-79a7-4d17-bfe9-989209d58781",
        text: "Каких объектов в Сосновом Бору вам не хватает?",
        type: "checkboxList",
        description: "Укажите 5 наиболее значимых объектов",
        data: [
            "Школы",
            "Детские сады",
            "Колледжи",
            "Университеты",
            "Медицинские учреждения",
            "Общественные пространства, парки и зеленые зоны",
            "Места общественного питания (кафе/рестораны)",
            "Спортивные объекты (спортзалы, клубы, бассейны, стадионы)",
            "Велодорожки",
            "Уличные спортивные площадки",
            "Культурные и досуговые объекты (театры, музеи, библиотеки, концертные залы)",
            "Торговые центры",
            "Развлекательные объекты (кинотеатры, парки аттракционов, развлекательные центры)",
            "Детские развлекательные объекты (площадки, игровые комнаты, детские клубы)",
            "Бытовые услуги (мастерские, салоны красоты, химчистки, ателье)",
            "Специализированные услуги (банки, нотариус, фото)",
            "Санатории, СПА - комплексы, бани",
            "Благоустроенные места на природе (места для рыбалок, купания и пляжного отдыха)",
            "Парковки и гаражи",
            "Всего хватает",
        ],
        maxValues: 5,
    },
    // tab 5
    {
        id: "e9cca138-da6a-46f8-a8ab-e567b07056b4",
        text: "Какой транспорт вы чаще всего используете для передвижения по Сосновому Бору?",
        type: "checkboxList",
        description: "Укажите 2 наиболее часто используемых варианта",
        data: [
            "Общественный транспорт (автобус, маршрутка и т.д.)",
            "Личный автомобиль или автомобиль семьи",
            "Автомобиль друзей/коллег/служебный",
            "Мопед/мотоцикл/скутер",
            "Велосипед/самокат/электросамокат",
            "Такси",
            "Хожу пешком",
        ],
        maxValues: 2,
    },
    {
        id: "ed1d6bdf-46cf-40f6-99c2-f993e973c1e6",
        text: "Как долго вы обычно добираетесь от дома до места работы/учебы?",
        type: "select",
        description:
            "Если вы не работаете или работаете удаленно, то укажите время до места, куда вы чаще всего ездите",
        data: ["Меньше 5 минут", "5 – 15 минут", "15 – 30 минут", "30 – 1 час", "1 час – 1,5 часа", "более 1,5 часов"],
    },
    {
        id: "ed17a9f7-38ff-42b5-a0c1-d0698a0b1736",
        text: "Вам легко и просто добраться до всех необходимых мест в Сосновом Бору?",
        type: "select",
        data: [
            "Полностью согласен",
            "Скорее согласен",
            "Скорее не согласен",
            "Полностью не согласен",
            "Затрудняюсь ответить",
        ],
    },
    {
        id: "01ff6364-bc21-4b5f-8534-26f7661f6d3c",
        text: "С какими проблемами общественного транспорта вы чаще всего сталкиваетесь в Сосновом Бору?",
        type: "checkboxList",
        description: "Укажите 5 наиболее значимых проблемы",
        data: [
            "Низкая частота отправления общественного транспорта",
            "Ограниченное время работы общественного транспорта",
            "Отсутствие расписания движения общественного транспорта",
            "Несоблюдение расписания общественным транспортом",
            "Низкая скорость движения общественного транспорта",
            "Переполненность общественного транспорта пассажирами",
            "Высокая стоимость проезда",
            "Неудобное расположение остановок общественного транспорта",
            "Неудовлетворительное состояние остановочных пунктов",
            "Невозможность оплаты проезда удобным способом (наличными и безналичным расчетом)",
            "Грубое обращение водителей общественного транспорта",
            "Не сталкивался с проблемами",
            "Не использую общественный транспорт в Сосновом Бору",
        ],
        maxValues: 5,
    },
    {
        id: "14f5a9ca-bb36-4e25-bfb2-c751007304e4",
        text: "С какими транспортными проблемами вы чаще всего сталкиваетесь в Сосновом Бору?",
        type: "checkboxList",
        description: "Укажите 4 наиболее значимых проблемы.",
        data: [
            "Пробки",
            "Плохое состояние покрытия дорог",
            "Низкое качество уборки дорог (от снега, мусора и др.)",
            "Недостаток парковочных мест",
            "Отсутствие выделенных полос для велосипедов/самокатов",
            "Отсутствие пешеходных тротуаров",
            "Отсутствие указателей и удобной навигации",
            "Не сталкиваюсь с проблемами",
        ],
        maxValues: 4,
    },
    {
        id: "4d8f34fe-c77c-4489-a7bc-999c324f2518",
        text: "Как часто вы выезжаете за пределы Соснового Бора?",
        type: "select",
        data: [
            "Ежедневно",
            "Несколько раз в неделю",
            "Несколько раз в месяц",
            "Несколько раз за полгода",
            "Реже 1-2 раз в год",
            "Никогда",
        ],
    },
    {
        id: "d35004b5-fc7b-4d02-a8a1-5be8be777292",
        text: "В какие населенные пункты вы выезжаете?",
        type: "textarea",
        description: "Это могут быть города или села, не входящие в Сосновый Бор, которые вы посещаете чаще всего",
    },
    {
        id: "2b919d67-437f-49ab-bd19-682fcf8e0003",
        text: "С какими целями Вы выезжаете за пределы Соснового Бора?",
        type: "checkboxOther",
        data: [
            "Работа, командировки",
            "Получение высшего образования",
            "Курсы повышения квалификации",
            "Занятия спортом",
            "Проведение отпуска или досуга",
            "Общение с родственниками/друзьями и знакомыми",
            "Проведение времени с детьми, посещение детских развлекательных центров и мероприятий",
            "Посещение медицинских учреждений",
            "Посещение магазинов",
            "Посещение кафе/баров/ресторанов",
            "Посещение культурных и развлекательных мероприятий",
        ],
    },
    {
        id: "7ae27223-ab62-46f6-8578-9ed00a2f4b5b",
        text: "Когда вы выезжаете за пределы Соснового Бора, каким транспортом вы обычно пользуетесь?",
        type: "checkboxOther",
        data: [
            "Общественный транспорт (автобус, маршрутка и т.д.)",
            "Личный автомобиль или автомобиль семьи",
            "Автомобиль друзей/коллег/служебный",
            "Поезд (пригородные электрички)",
            "Мопед/мотоцикл/скутер",
            "Такси",
            "Велосипед",
        ],
    },
    // tab 6
    {
        id: "a047c0a3-3424-4fe3-9976-cc1886671e1a",
        text: "Вы бы переехали из Соснового Бора, если бы представилась такая возможность?",
        type: "select",
        data: [
            "Да, планирую уехать через 1–2 года",
            "Да, хотел(-а) бы уехать в будущем",
            "Нет, хочу остаться жить в Сосновом Бору",
            "Затрудняюсь ответить",
        ],
    },
    {
        id: "a206f0b0-6cf1-4193-befc-e8165f0ef930",
        text: "Почему вы хотите переехать из Соснового Бора?",
        type: "textarea",
    },
    {
        id: "7ab00439-bec4-47d3-9c1b-7238864f03a2",
        text: "Каким вы хотели бы видеть Сосновый Бор в ближайшие 10 лет?",
        type: "textarea",
        description:
            "Вы можете написать самые смелые пожелания по развитию города и конкретные предложения по развитию городских пространств и учреждений",
    },
    {
        id: "8537aa0d-7b89-4104-b033-242018f0d70e",
        text: "Какие особенности жизни в Сосновом Бору вы бы хотели сохранить?",
        type: "textarea",
    },
    {
        id: "6d1aa949-e159-4fbe-8a6c-d780ad1946a9",
        text: "Если у Вас есть дети или внуки школьного возраста, каким вы видите их профессиональное будущее?",
        type: "select",
        data: [
            "Получат образование в Сосновом Бору и останутся здесь работать",
            "Получат образование в Сосновом Бору и уедут работать в другой город",
            "Уедут учиться в другой город и вернутся в Сосновый Бор после получения диплома",
            "Уедут учиться в другой город и останутся работать в других городах России",
            "Не будут получать профессиональное образование и будут работать в Сосновом Бору",
            "Не будут получать профессиональное образование и уедут работать в другой город",
            "Нет детей или внуков школьного возраста",
        ],
    },
    {
        id: "6875f942-182c-41d2-ba4b-0f33b9e7c4e7",
        text: "Как вы думаете, какие направления стоит развивать в первую очередь в Сосновом Бору?",
        type: "checkboxList",
        description: "Выберите не более 4-х вариантов.",
        data: [
            "Образование",
            "Благоустройство городских пространств",
            "Благоустройство двора и прилегающей к дому территории",
            "Развитие общественного транспорта",
            "Доступность и качество жилья, коммунальных услуг",
            "Повышение социальной активности граждан, создание городского сообщества",
            "Развитие информационных ресурсов о жизни города",
            "Социальная поддержка населения",
            "Физическая культура и спорт",
            "Культура и досуг",
            "Рынок труда и карьерные возможности",
            "Сотовая и интернет-связь",
            "Здравоохранение",
            "Промышленное производство",
            "Малое и среднее предпринимательство",
            "Экология",
            "Туризм",
        ],
        maxValues: 4,
    },
    // tab 7
    {
        id: "83aab121-8c9c-47ec-83f6-68cdb3b21dd7",
        text: "Вы проживаете в Сосновом Бору с рождения или переехали в него?",
        type: "select",
        data: [
            "Проживаю с рождения",
            "Переехал",
            "Проживаю временно (приехал (-а) на работу / по другим обстоятельствам)",
        ],
    },
    {
        id: "05124afe-8822-4785-9bd7-e837e0fb8c16",
        text: "Где вы проживали до переезда в Сосновый Бор?",
        type: "textarea",
    },
    {
        id: "45d21c99-3563-4f25-9fe3-59f4a1398b32",
        text: "Сколько лет вы проживаете в Сосновом Бору?",
        type: "textarea",
        description: "Если вы уезжали из города и вернулись, отметьте суммарное количество прожитых в городе лет",
    },
    {
        id: "5b1b2973-4307-49b9-b2b1-eb5e85fb5e84",
        text: "Где вы проживаете?",
        type: "select",
        data: [
            "В собственной квартире",
            "В собственном доме",
            "Живу в квартире/доме родителей",
            "В арендованном жилье",
            "В арендованном жилье с компенсацией проживания",
        ],
    },
    {
        id: "3a9bd4d5-c99e-476a-9418-9b0538a7f16f",
        text: "Укажите ваш пол",
        type: "select",
        data: ["Мужской", "Женский"],
    },
    {
        id: "25fecdd4-fcb7-46f0-ab25-3ca248eefc92",
        text: "Сколько вам полных лет?",
        type: "textarea",
    },
    {
        id: "5c8b0a36-ddd8-40e9-8310-30da4c76d8d7",
        text: "Укажите ваше семейное положение",
        type: "select",
        data: ["Холост", "Есть партнер", "Состою в официальном браке", "Разведен(-а)", "Вдовец/вдова"],
    },
    {
        id: "6ee6c69f-bbff-406f-97ed-3c53d149e63f",
        text: "У вас есть дети до 18 лет?",
        type: "select",
        data: ["Нет", "1 ребенок", "2 ребенка", "3 ребенка и более"],
    },
    {
        id: "0b1fd449-7999-4652-a54c-b9defb863317",
        text: "Укажите ваш уровень образования",
        type: "select",
        description: "Укажите уровень, который Вы уже окончили",
        data: [
            "Основное общее образование (5–9 классы)",
            "Среднее общее образование (10–11 классы)",
            "Среднее профессиональное образование (колледж, техникум)",
            "Высшее образование (бакалавриат, магистратура, специалитет)",
            "Ученая степень",
        ],
    },
    {
        id: "06e18b7b-8646-41cc-84f1-9532db7d8d7d",
        text: "Какое высказывание точнее всего описывает материальное положение вашей семьи?",
        type: "select",
        data: [
            "Денег не хватает на продукты питания",
            "На продукты денег хватает, но покупка одежды вызывает финансовые затруднения",
            "Денег хватает на продукты и одежду, но покупка крупной бытовой техники (телевизора, холодильника) является проблемой",
            "Могу без труда покупать крупную бытовую технику (телевизор, холодильник), но на дорогие вещи (автомобиль, квартиру) денег не хватает",
            "Могу позволить себе достаточно дорогостоящие вещи (автомобиль, квартиру, дачу и др.)",
        ],
    },
    {
        id: "676e815a-436e-40a8-8b2b-05efb50f667b",
        text: "Какой ваш основной род занятий?",
        type: "checkboxOther",
        data: [
            "Наемный работник (специалист)",
            "Наемный работник (руководитель)",
            "Предприниматель, владелец бизнеса",
            "Самозанятый/фрилансер",
            "Студент/учащийся",
            "Декретный отпуск / занят(-а) домашним хозяйством",
            "Не работающий пенсионер",
            "Работающий пенсионер",
            "Временно безработный/в поиске работы",
        ],
    },
    {
        id: "59524fc1-1588-46ce-aaef-0509d473eea9",
        text: "В какой отрасли вы работаете?",
        type: "checkboxOther",
        data: [
            "Электроэнергетика",
            "Банки, страхование, финансы, бухгалтерия",
            "Государственная и муниципальная служба",
            "Информационные технологии",
            "Красота, фитнес, спорт",
            "Культура и искусство",
            "Машиностроение",
            "Медицина и здравоохранение",
            "Наука",
            "Образование",
            "Сельское хозяйство и производство продуктов питания",
            "Средства массовой информации, медиа и маркетинг",
            "Строительство и недвижимость",
            "Торговля",
            "Транспорт и логистика",
            "Туризм и досуг",
            "Управление персоналом, кадры",
            "Юриспруденция",
            "Я не работаю",
        ],
    },
]

type SchemaToComponentsProps = {
    schema: SurveySchemaItem[]
    formHook: UseFormReturn<SurveyFormData>
    renderFilter?: Record<string, boolean>
}
function SchemaToComponents({ schema, formHook, renderFilter = {} }: SchemaToComponentsProps) {
    const { control, setValue, watch, register } = formHook // passed from parent cause it needed for filter
    return schema
        .filter((item) => renderFilter?.[item.id] ?? true)
        .map((item) => {
            switch (item.type) {
                case "select": {
                    return (
                        <Controller
                            name={item.id}
                            control={control}
                            rules={{ required: true }}
                            render={({ field }) => (
                                <Select
                                    {...field}
                                    value={typeof field.value === "string" ? field.value : null}
                                    label={item.text}
                                    data={item.data ?? []}
                                    description={item?.description}
                                    styles={{
                                        wrapper: {
                                            "--input-bd-focus": "var(--mantine-color-secondary-0)",
                                        },
                                    }}
                                />
                            )}
                        />
                    )
                }
                case "checkboxOther": {
                    return (
                        <Controller
                            name={item.id}
                            control={control}
                            rules={{ required: true }}
                            render={({ field }) => (
                                <CheckboxWithOther
                                    field={field}
                                    setValue={setValue}
                                    watch={watch}
                                    label={item.text}
                                    description={item?.description}
                                    maxValues={item?.maxValues}
                                    data={item.data ?? []}
                                />
                            )}
                        />
                    )
                }
                case "textarea": {
                    return (
                        <Stack>
                            {item.id === "a7b52f44-cc92-4df5-bf26-f45a66fe5cb0" && (
                                <div
                                    style={{
                                        overflow: "hidden",
                                    }}
                                >
                                    <Image
                                        src={"/question.jpg"}
                                        alt=""
                                        width={2950}
                                        height={2741}
                                        style={{
                                            width: "100%",
                                            height: "auto",
                                            aspectRatio: "2950 / 2741",
                                            scale: 1.5,
                                        }}
                                    />
                                </div>
                            )}
                            <Textarea
                                {...register(item.id, { required: true })}
                                label={item.text}
                                rows={item?.rows ?? 3}
                                description={item?.description}
                                styles={{
                                    wrapper: {
                                        "--input-bd-focus": "var(--mantine-color-secondary-0)",
                                    },
                                }}
                            />
                        </Stack>
                    )
                }
                case "selectList": {
                    return (
                        <Fieldset legend={item.text} variant="filled">
                            <Stack>
                                {(item.list ?? []).map((listItem: string, index: number) => (
                                    <Controller
                                        key={listItem}
                                        name={`${item.id}-${index}`}
                                        control={control}
                                        rules={{ required: true }}
                                        render={({ field }) => (
                                            <Select
                                                {...field}
                                                value={typeof field.value === "string" ? field.value : null}
                                                label={listItem}
                                                styles={{
                                                    label: {
                                                        fontWeight: "lighter",
                                                    },
                                                    wrapper: {
                                                        "--input-bd-focus": "var(--mantine-color-secondary-0)",
                                                    },
                                                }}
                                                data={item.data ?? []}
                                            />
                                        )}
                                    />
                                ))}
                            </Stack>
                        </Fieldset>
                    )
                }
                case "sliderList": {
                    return (
                        <Fieldset
                            legend={item.text}
                            variant="filled"
                            styles={{
                                legend: {
                                    fontWeight: "bold",
                                },
                            }}
                        >
                            <Text
                                c="dimmed"
                                style={{
                                    fontSize: "unset",
                                }}
                            >
                                {item?.description}
                            </Text>
                            <Stack>
                                {(item.list ?? []).map((listItem: string, index: number) => (
                                    <Controller
                                        key={listItem}
                                        name={`${item.id}-${index}`}
                                        control={control}
                                        // rules={{ required: true }}
                                        render={({ field }) => (
                                            <Box my="md">
                                                <Text
                                                    style={{
                                                        fontSize: "unset",
                                                    }}
                                                >
                                                    {listItem}
                                                </Text>
                                                <Slider
                                                    {...field}
                                                    value={typeof field.value === "number" ? field.value : 0}
                                                    step={1}
                                                    min={0}
                                                    max={(item.marks ?? []).length - 1}
                                                    defaultValue={0}
                                                    marks={(item.marks ?? []).map((x: number) => ({
                                                        value: x,
                                                        label: String(x),
                                                    }))}
                                                    label={null}
                                                    styles={{
                                                        root: {
                                                            "--slider-color": "var(--mantine-color-secondary-0)",
                                                        },
                                                    }}
                                                />
                                            </Box>
                                        )}
                                    />
                                ))}
                            </Stack>
                        </Fieldset>
                    )
                }
                case "checkboxList": {
                    return (
                        <Controller
                            name={item.id}
                            control={control}
                            rules={{ required: true }}
                            render={({ field }) => (
                                <CheckboxList
                                    field={field}
                                    watch={watch}
                                    label={item.text}
                                    description={item?.description}
                                    maxValues={item?.maxValues}
                                    data={item.data ?? []}
                                />
                            )}
                        />
                    )
                }
                default: {
                    console.log("schema map invalid item. id:", item?.id)
                    return null
                }
            }
        })
}

function Tab1({ onSubmit, submitText, onSubmitData }: TabProps): React.ReactElement {
    const formHook = useForm()
    const { handleSubmit, control } = formHook
    const onSubmitWrapped = async (data: SurveyFormData) => {
        if (questoin0Value === "Нет") {
            onSubmitData?.(data)
            return
        }
        onSubmit?.(data)
    }

    const [questoin0Value] = useWatch({
        control,
        name: [schema[0].id],
    })

    const renderFilter = {
        [schema[1].id]: questoin0Value === "Да",
        [schema[2].id]: questoin0Value === "Нет",
        [schema[3].id]: questoin0Value === "Нет",
    }

    return (
        <form onSubmit={handleSubmit(onSubmitWrapped)}>
            <Stack>
                <SchemaToComponents schema={schema.slice(0, 4)} formHook={formHook} renderFilter={renderFilter} />
                <Group align="flex-start">
                    <Button type="submit" disabled={submitText === states.fetch} bg="secondary">
                        {questoin0Value === "Нет" ? submitText : "Следующий раздел"}
                    </Button>
                </Group>
            </Stack>
        </form>
    )
}

function Tab2({ onSubmit = () => {}, setTabIndex }: TabProps): React.ReactElement {
    const formHook = useForm()
    const { handleSubmit } = formHook
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            <Stack>
                <SchemaToComponents schema={schema.slice(4, 10)} formHook={formHook} />

                <Group align="flex-start">
                    <Button onClick={() => setTabIndex?.((x) => x - 1)} bg="secondary">
                        Предыдущий раздел
                    </Button>
                    <Button type="submit" bg="secondary">
                        Следующий раздел
                    </Button>
                </Group>
            </Stack>
        </form>
    )
}

function Tab3({ onSubmit = () => {}, setTabIndex }: TabProps): React.ReactElement {
    const formHook = useForm()
    const { handleSubmit } = formHook
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            <Stack>
                <SchemaToComponents schema={schema.slice(10, 16)} formHook={formHook} />

                <Group align="flex-start">
                    <Button onClick={() => setTabIndex?.((x) => x - 1)} bg="secondary">
                        Предыдущий раздел
                    </Button>
                    <Button type="submit" bg="secondary">
                        Следующий раздел
                    </Button>
                </Group>
            </Stack>
        </form>
    )
}

function Tab4({ onSubmit = () => {}, setTabIndex }: TabProps): React.ReactElement {
    const formHook = useForm()
    const { handleSubmit, control } = formHook

    const [questoinValue] = useWatch({
        control,
        name: [schema[24].id],
    })

    const renderFilter = {
        [schema[25].id]: questoinValue === "Никогда",
    }

    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            <Stack>
                <SchemaToComponents schema={schema.slice(16, 19)} formHook={formHook} renderFilter={renderFilter} />

                <Group align="flex-start">
                    <Button onClick={() => setTabIndex?.((x) => x - 1)} bg="secondary">
                        Предыдущий раздел
                    </Button>
                    <Button type="submit" bg="secondary">
                        Следующий раздел
                    </Button>
                </Group>
            </Stack>
        </form>
    )
}

function Tab5({ onSubmit = () => {}, setTabIndex }: TabProps): React.ReactElement {
    const formHook = useForm()
    const { handleSubmit, control } = formHook

    const [questoinValue] = useWatch({
        control,
        name: [schema[24].id],
    })

    const renderFilter = {
        [schema[25].id]: [
            "Ежедневно",
            "Несколько раз в неделю",
            "Несколько раз в месяц",
            "Несколько раз за полгода",
            "Реже 1-2 раз в год",
        ].includes(questoinValue),
    }

    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            <Stack>
                <SchemaToComponents schema={schema.slice(19, 28)} formHook={formHook} renderFilter={renderFilter} />

                <Group align="flex-start">
                    <Button onClick={() => setTabIndex?.((x) => x - 1)} bg="secondary">
                        Предыдущий раздел
                    </Button>
                    <Button type="submit" bg="secondary">
                        Следующий раздел
                    </Button>
                </Group>
            </Stack>
        </form>
    )
}

function Tab6({ onSubmit = () => {}, setTabIndex }: TabProps): React.ReactElement {
    const formHook = useForm()
    const { handleSubmit, control } = formHook

    const [questoinValue] = useWatch({
        control,
        name: [schema[28].id],
    })

    const renderFilter = {
        [schema[29].id]: ["Да, планирую уехать через 1–2 года", "Да, хотел(-а) бы уехать в будущем"].includes(
            questoinValue,
        ),
        [schema[30].id]: ["Да, планирую уехать через 1–2 года", "Да, хотел(-а) бы уехать в будущем"].includes(
            questoinValue,
        ),
    }

    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            <Stack>
                <SchemaToComponents schema={schema.slice(28, 34)} formHook={formHook} renderFilter={renderFilter} />

                <Group align="flex-start">
                    <Button onClick={() => setTabIndex?.((x) => x - 1)} bg="secondary">
                        Предыдущий раздел
                    </Button>
                    <Button type="submit" bg="secondary">
                        Следующий раздел
                    </Button>
                </Group>
            </Stack>
        </form>
    )
}

function Tab7({ setTabIndex, globalFormValues, onSubmitData, submitText }: TabProps): React.ReactElement {
    const formHook = useForm()
    const { handleSubmit, control } = formHook
    const onSubmit = async (data: SurveyFormData) => {
        onSubmitData?.({ ...globalFormValues, ...data })
        return
    }

    const [questoinValue] = useWatch({
        control,
        name: [schema[34].id],
    })

    const renderFilter = {
        [schema[35].id]: [
            "Переехал",
            "Проживаю временно (приехал (-а) на работу / по другим обстоятельствам)",
        ].includes(questoinValue),
        [schema[36].id]: ["Проживаю с рождения"].includes(questoinValue),
    }
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            <Stack>
                <SchemaToComponents schema={schema.slice(34, 46)} formHook={formHook} renderFilter={renderFilter} />

                <Group align="flex-start">
                    <Button onClick={() => setTabIndex?.((x) => x - 1)} bg="secondary">
                        Предыдущий раздел
                    </Button>
                    <Button type="submit" disabled={submitText === states.fetch} bg="secondary">
                        {submitText}
                    </Button>
                </Group>
            </Stack>
        </form>
    )
}

export function SurveyModal() {
    const modals = useModals()
    const [tabIndex, setTabIndex] = useState(0)
    const [globalFormValues, setGlobalFormValues] = useState<SurveyFormData | null>(null)

    const ref = useRef<HTMLDivElement>(null)

    const [text, setText] = useState(states.start)

    const titles: Record<number, string> = {
        0: "Приветствие",
        1: "Общие вопросы",
        2: "Идентичность Соснового Бора",
        3: "Досуг",
        4: "Городская среда",
        5: "Транспорт",
        6: "Планы на будущее",
        7: "Социально-демографические характеристики",
    }

    const onSubmitStep = async (data: SurveyFormData) => {
        setGlobalFormValues((x) => ({ ...x, ...data }))
        setTabIndex((x) => x + 1)
        ref.current?.scrollIntoView()
    }

    const getSelectListText = useCallback((key: string) => {
        // its a selectList question with "id-i" pattern
        const splited = key.split("-")
        const i = splited.pop()
        const id = splited.join("-")
        const schemaQuestion = schema.find((x) => id === x.id)

        return schemaQuestion ? `${schemaQuestion.text}: ${schemaQuestion.list?.[Number(i)]}` : ""
    }, [])

    const onSubmitData = async (data: SurveyFormData) => {
        const body = JSON.stringify({
            data: Object.entries(data)
                .filter(([_, value]) => !!value)
                .map(([key, value]) => ({
                    id: key,
                    text: schema.find((x) => x.id === key)?.text ?? getSelectListText(key),
                    value,
                })),
        })

        await fetch(`/api/collections/surveys/records`, {
            method: "post",
            body,
            headers: {
                "Content-Type": "application/json",
            },
        })
            .then(async (res) => {
                const isResOk = res.status < 300

                setText(isResOk ? states.ok : states.error)

                if (!isResOk) {
                    return
                }
                modals.closeContextModal("survey")
                modals.openModal({
                    centered: true,
                    withCloseButton: false,
                    children: (
                        <Stack>
                            <Center>
                                <Title order={1}>
                                    <Text pt="lg" inherit>
                                        Спасибо!
                                    </Text>
                                </Title>
                            </Center>
                            <Center>
                                <Text size="xl" ta={"center"}>
                                    Опрос окончен.
                                </Text>
                            </Center>
                        </Stack>
                    ),
                    onClose: () => modals.closeAll(),
                })
            })
            .catch(async (e) => {
                setText(states.error)
                console.error("Survey submission failed:", e)
            })
    }

    return (
        <Tabs
            value={String(tabIndex)}
            onChange={() => null}
            variant="pills"
            color="secondary"
            styles={{
                tab: {
                    borderRadius: "40px",
                },
            }}
        >
            <div ref={ref} />
            <div style={{ display: "flex", justifyContent: "space-between" }}>
                <Text size="lg" fw="bold">
                    {tabIndex === 0
                        ? titles[tabIndex]
                        : `${tabIndex}/${Object.keys(titles).length - 1} ${titles[tabIndex]} `}
                </Text>
                <CloseButton radius={"40px"} onClick={modals.closeAll} />
            </div>

            <Space h="xl" />

            <Tabs.Panel value="0">
                <Stack gap="md">
                    <Text size="xl" fw="bold">
                        Уважаемый участник опроса!
                    </Text>
                    <Text>
                        Администрация Соснового Бора совместно с госкорпорацией «Росатом» разрабатывает Мастер-план г.
                        Сосновый Бор, который позволит определить общие принципы и направления развития территории.
                        <br />
                        <br />
                        Нам важно узнать, что волнует жителей города и как должен развиваться Сосновый Бор в будущем.
                        Ваши предложения и пожелания будут учтены в разработке Мастер-плана.
                        <br />
                        <br />
                        Анкета содержит перечень вопросов о современном состоянии городской среды и перспективах
                        развития территории. Заполнение анкеты займет около 15 минут. Мы не собираем личные данные
                        участников опроса и гарантируем полную конфиденциальность.
                        <br />
                        <br />
                    </Text>
                    <Center>
                        <Button
                            bg="secondary"
                            onClick={() => {
                                setTabIndex((x) => x + 1)
                                ref.current?.scrollIntoView()
                            }}
                        >
                            Начать опрос
                        </Button>
                    </Center>
                </Stack>
            </Tabs.Panel>
            <Tabs.Panel value="1">
                <Tab1 onSubmit={onSubmitStep} submitText={text} onSubmitData={onSubmitData} />
            </Tabs.Panel>
            <Tabs.Panel value="2">
                <Tab2 onSubmit={onSubmitStep} setTabIndex={setTabIndex} />
            </Tabs.Panel>
            <Tabs.Panel value="3">
                <Tab3 onSubmit={onSubmitStep} setTabIndex={setTabIndex} />
            </Tabs.Panel>
            <Tabs.Panel value="4">
                <Tab4 onSubmit={onSubmitStep} setTabIndex={setTabIndex} />
            </Tabs.Panel>
            <Tabs.Panel value="5">
                <Tab5 onSubmit={onSubmitStep} setTabIndex={setTabIndex} />
            </Tabs.Panel>
            <Tabs.Panel value="6">
                <Tab6 onSubmit={onSubmitStep} setTabIndex={setTabIndex} />
            </Tabs.Panel>
            <Tabs.Panel value="7">
                <Tab7
                    setTabIndex={setTabIndex}
                    globalFormValues={globalFormValues ?? undefined}
                    onSubmitData={onSubmitData}
                    submitText={text}
                />
            </Tabs.Panel>
        </Tabs>
    )
}
