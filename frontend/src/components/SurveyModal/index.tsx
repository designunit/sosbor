...
const schema = [
    ...
    },
    {
        ...
        "type": "textarea",
        "description": "Это могут быть любые значимые для вас или просто интересные особенности города (в том числе в людях), которые отличают Сосновый Бор от других городов"
    },
    ...
    },
    {
        ...
        "type": "textarea",
        "description": "Вы можете написать самые смелые пожелания по развитию города и конкретные предложения по развитию городских пространств и учреждений"
    },
    {
...
function Tab6({ onSubmit = () => {}, setTabIndex }: TabProps): React.ReactElement {
    ...
    const renderFilter = {
        [schema[29].id]: ['Да, планирую уехать через 1–2 года', 'Да, хотел(-а) бы уехать в будущем'].includes(questoinValue),
        [schema[30].id]: ['Да, планирую уехать через 1–2 года', 'Да, хотел(-а) бы уехать в будущем'].includes(questoinValue),
    }
...
function Tab7({ setTabIndex, globalFormValues, onSubmitData, submitText }: TabProps): React.ReactElement {
    ...
    const renderFilter = {
        [schema[35].id]: ['Переехал', 'Проживаю временно (приехал (-а) на работу / по другим обстоятельствам)'].includes(questoinValue),
...
export function SurveyModal() {
    ...
    const onSubmitData = async (data: SurveyFormData) => {
        ...
        await fetch(
            `/api/collections/surveys/records`,
            {
                method: 'post',
                body,
                headers: {
        ...
        >
            ...
            <Tabs.Panel value='0'>
                <Stack gap='md'>
                    ...
                    <Text>
                        Администрация Соснового Бора совместно с госкорпорацией «Росатом» разрабатывает Мастер-план г. Сосновый Бор, который позволит определить общие принципы и направления развития территории.<br /><br />
                        Нам важно узнать, что волнует жителей города и как должен развиваться Сосновый Бор в будущем. Ваши предложения и пожелания будут учтены в разработке Мастер-плана.<br /><br />
                        Анкета содержит перечень вопросов о современном состоянии городской среды и перспективах развития территории. Заполнение анкеты займет около 15 минут. Мы не собираем личные данные участников опроса и гарантируем полную конфиденциальность.<br /><br />
                    </Text>
                    <Center>
                        <Button
                            bg='secondary'
